...
def mostrar_clima_actual():
    ...
    tk.Label(frame_actual, text=f"{temp} {unidad_temp}", bg="white", font=("Arial", 22, "bold")).place(relx=10/640, rely=35/480)
...
def mostrar_pronostico():
    ...
    for item in data_pro["list"][:8]:
        shift_segundos = data_pro["city"]["timezone"]
        hora_utc = datetime.fromisoformat(item["dt_txt"])
        hora = hora_utc + timedelta(seconds=shift_segundos)
        temp = item["main"]["temp"]
        if opciones["system"]=="imperial":
            temp = temp*9/5 + 32
        ...
        unidad_temp = "°C" if opciones["system"]=="metric" else "°F"
        temp = round(temp)
        tk.Label(contenedor, text=f"{hora.strftime('%H:%M')}: {temp}{unidad_temp} - {desc}", bg="white", font=("Arial", 13)).pack(anchor="w", padx=20)
    ...
    for fecha, temps in list(dias.items())[:5]:
        ...
        tk.Label(contenedor, text=f"{fecha_fmt}: {textopronosticominimo} {temp_min}{unidad_temp} / {textopronosticomaximo} {temp_max}{unidad_temp}", bg="white", font=("Arial", 13)).pack(anchor="w", padx=20)
